---
title: "icpsrdata: Reproducible Retrieval of Pew Research Center Datasets"
...
`icpsrdata` depends on the Firefox browser; if it is not installed, [get it here](https://www.mozilla.org/firefox).

An optional, but recommended, setup step is to add the information ICPSR requires to your [.Rprofile](http://www.statmethods.net/interface/customizing.html) as in the following example:
...
The `icpsr_download` function (1) opens a Firefox browser pointed to a specified dataset, (2) enters the required information and agrees to the terms and conditions of the download, (3) downloads the dataset's files, and, optionally but by default, (4) decompresses the dataset's files to a subdirectory within a specified directory (or, by default, a `icpsr_data` directory in the current working directory).

Datasets are specified using the `file_id` argument.  ICPSR uses a unique number to identify each of its datasets; this number is consistently listed alongside the dataset's name.  For the [2014 Transatlantic Trends Survey](http://www.icpsr.umich.edu/icpsrweb/ICPSR/series/235/studies/36138), for example, the file id is 36138:
